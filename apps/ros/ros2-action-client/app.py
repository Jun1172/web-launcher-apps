...
import json, os, subprocess, threading, time, shlex
...
def get_action_type(name):
    """获取动作类型并清理格式"""
    result = run_cmd(f"ros2 action type {name}")
    if result:
        # 清理换行符和多余空格，只取第一行
...
def send_goal(name, action_type, args):
    ...
    def worker():
        try:
            # 清理动作类型
            action_type_clean = action_type.strip() if action_type else ""
            
            # 安全转义参数
            safe_name = shlex.quote(name)
            safe_type = shlex.quote(action_type_clean)
            safe_args = shlex.quote(args)
            
            cmd = f'ros2 action send_goal {safe_name} {safe_type} {safe_args} --feedback'
            active_goals[goal_id]["log"].append(f"▶ 执行命令: {cmd}")
            
            # 关键修复：合并 stderr 到 stdout
            ...
            for line in proc.stdout:
                if goal_id in active_goals:
                    line = line.strip()
                    if line:
                        active_goals[goal_id]["log"].append(line)
            ...
            if goal_id in active_goals:
                if proc.returncode == 0:
                    ...
                    active_goals[goal_id]["log"].append("✅ 执行成功")
                else:
                    active_goals[goal_id]["status"] = "error"
                    active_goals[goal_id]["log"].append(f"❌ 命令失败，退出码: {proc.returncode}")
                    
        except Exception as e:
            if goal_id in active_goals:
                active_goals[goal_id]["log"].append(f"❌ 异常: {str(e)}")
                active_goals[goal_id]["status"] = "error"
...
<div class="container">
  <h2> ROS2 动作执行器</h2>
  <div class="tip"><strong>💡 使用提示：</strong>没有动作可执行？请先启动动作服务器：执行 <code>ros2 run action_tutorials_cpp fibonacci_action_server</code>，或打开 <a href="http://127.0.0.1:8200/" target="_blank">ROS2 演示节点</a> 启动「动作服务器」。目标参数示例：<code>{order: 5}</code>。</div>
  <div class="card">
    <label>选择动作</label>
