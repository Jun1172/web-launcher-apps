...
def start_demo_node(name, script_name):
    ...
    try:
        ...
        proc = subprocess.Popen(
            ...
            env=env,
            creationflags=subprocess.CREATE_NO_WINDOW if platform.system() == "Windows" else 0 # Windows 下隐藏黑框
        )
        processes[name] = proc
        errors[name] = ""
...
.btn-all{padding:10px 20px;margin:5px;background:#059669;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:bold}
...
<div class="container">
  ...
  <div class="card">
    <h3>批量控制</h3>
    <button class="btn-all" onclick="startAll()">🚀 启动所有节点</button>
    <button class="btn-all" onclick="stopAll()">⏹️ 停止所有节点</button>
  </div>
  <div class="card">
    <h3>节点列表</h3>
...
class H(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/api/status':
        ...
        elif parsed.path == '/api/start_all':
            for n in nodes_config:
                start_demo_node(n['id'], n['script'])
            self.send_response(200)
            self.send_header("Content-Type", "text/plain")
            self.end_headers()
            self.wfile.write("已发送启动所有节点指令".encode())
            ...
            self.send_header("Content-Type", "text/plain")
            self.end_headers()
            self.wfile.write("已停止所有节点".encode())
